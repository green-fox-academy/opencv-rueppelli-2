...
[Normalize coloring] (# Normalize-coloring)
...
```cv::Mat binarizeImageOtsu(cv::Mat image)```
   - Purpose of the function:
     - it binarize the input image, based on the number of black and white pixels it return the thresholded image or its invert
   - Parameter:
     - image: input image
   - Returntype: cv::Mat object
...
```cv::Mat normalizeColoring(cv::Mat img)```
  - Purpose of the function:
    - normalizes the coloring of the input image
  - Parameter:
    - img: input image
  - Returntype: cv:: Mat object

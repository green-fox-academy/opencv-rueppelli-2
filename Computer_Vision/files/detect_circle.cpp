...
cv::Mat detectCircle()
{
    if(!blurredImage.empty()) {
        ...
        HoughCircles(blurredImage, circles, cv::HOUGH_GRADIENT, 1, blurredImage.rows/6, 100, 30, 0, 0);
        ...
        for(size_t i = 0; i < circles.size(); i++)
        {
            ...
            cv::putText(image, "X: " + xString, cv::Point(centerX - r/4, centerY + r/3), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,0,255), 2, 8, false);

            std::string yString = std::to_string(centerY);
            cv::putText(image, "Y: " + yString, cv::Point(centerX - r/4, centerY + r/2 + r/6), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,0,255), 2, 8, false);
        }
    }
    return image;
}
